...
def match_templates(rank_symbol, suit_symbol, rank_templates, suit_templates):
    ...
    suit_symbol_dilated = cv2.dilate(suit_symbol, kernel)
    cv2.imshow("Dilated Suit Symbol", suit_symbol_dilated)
    cv2.waitKey(0)
    ...
    for suit_name, suit_template in suit_templates.items():
        ...
        cv2.imshow("Suit Template", suit_template)

        suit_template_anded = cv2.bitwise_and(suit_template, suit_symbol_dilated)
        cv2.imshow("Suit Template AND Symbol Dilated", suit_template_anded)
        cv2.waitKey(0)

        # res = cv2.matchTemplate(suit_symbol_eroded, suit_template_eroded, cv2.TM_CCOEFF_NORMED)
        res = cv2.matchTemplate(suit_symbol, suit_template_anded, cv2.TM_CCOEFF_NORMED)
        score = res[0][0]
...
if __name__ == "__main__":
    image_path = 'TEMP/Project4/test_images3/6C.JPG'
