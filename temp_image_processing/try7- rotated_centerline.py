...
def straighten_worm(segmented_worm, parent=None):
    ...
    # -------------------- Step 7: Overlay Centerline on Rotated Image -------------------- #
    ...
    # Display the final rotated image with centerline
    display_results("Rotated Worm with Centerline", rotated, scale=2, parent=parent)

    print("straighten_worm function completed with rotation and centerline overlay.")
    return rotated
...
def process_video(video_path, parent=None):
    ...
    # Straighten the worm
    straightened_worm = straighten_worm(segmented_worm, parent=parent)
    if straightened_worm is None:
        return

    # Display the final result in the GUI
    display_result(frame, straightened_worm)
...
def load_video():
    ...
    if video_path:
        process_video(video_path, parent=root)
