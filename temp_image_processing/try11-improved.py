...
def refine_endpoints(backbone_cp, worm_mask):
    ...
    For instance, measure worm width at endpoints and see if adjustments are needed.
    This is a heuristic example and may need tuning for your data.
    """
    ...
    # If one end is much wider than the other, you might decide to trim a few points.
    # For example, if the start is very wide and end is narrow, maybe remove a few points from the start.
    # For now, let's just print info and not trim.
    ...
    # Currently, just return the original backbone.
    return backbone_cp
...
def main():
    global root
    root = tk.Tk()
    root.title("C. elegans Worm Segmentation, BDB+ Backbone Detection and Straightening (with Endpoint Refinement)")
